...
| 📊 **CRIPI Dashboard** | `pages/1_CRIPI_Dashboard.py` | KPIs, Mexico map, state/national indicator profiles, dimension rankings, priority gap chart, AI assistant |
| 🧩 **State Clusters** | `pages/2_State_Clusters.py` | PCA cluster map, cluster dimension profile, indicator deltas, clusters vs. priority chart, AI assistant |
...
The chat panel on the dashboard and cluster pages uses the **Grok API** (`XAI_API_KEY`).
...
Separate chat histories per page (`priorities` / `clusters`). See `ai/agent.py` for the two-step tool loop.
